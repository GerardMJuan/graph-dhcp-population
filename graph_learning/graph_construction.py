...
class PopulationGraphUKBB:
    def __init__(self, train_data, val_data, test_data, phenotype_columns, columns_kept, num_node_features, task, num_classes, k, edges):
    ...
    def load_data(self):
        ...
        data_df_train = self.train_data
        data_df_val = self.val_data
        data_df_test = self.test_data
        ...
        if self.task == 'classification':        
                ...
                df['gestational_weeks'] = pd.qcut(df['gestational_weeks'], q=self.num_classes, labels=labels).astype('int') #Balanced classes
