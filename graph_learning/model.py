...
from graph_learning.layers import *
...
class GraphLearningModel(pl.LightningModule):
    ...
    def predict_step(self, batch, batch_idx, dataloader_idx=None):
        """
        Predicts the output of the model for a given batch.
        Boilerplate code to be used in the predict method of the
        LightningModule.
        """
        X, y, mask, phenotypes, edges = batch
        edges = edges[0]
        mask=mask[0]
        ...
        if self.task == 'classification':
            # Get the predicted labels applying the mask
            pred = pred[:,mask.to(torch.bool),:]
